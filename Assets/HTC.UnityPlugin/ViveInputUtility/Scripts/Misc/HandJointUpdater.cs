...
namespace HTC.UnityPlugin.Vive
{
    public class HandJointUpdater : MonoBehaviour, RenderModelHook.ICustomModel, IViveRoleComponent
    {
        ...
        private float CalculateModelLength()
        {
            ...
            foreach (var t in m_modelJoints.ElementsFrom(HandJointName.MiddleMetacarpal, HandJointName.MiddleTip))
            {
                if (t != null)
                {
        ...
        {
            foreach (var t in m_modelJoints.ElementsFrom(HandJointName.MiddleTip, HandJointName.MiddleMetacarpal)) { if (t != null) { return t; } }
            foreach (var t in m_modelJoints.ElementsFrom(HandJointName.IndexTip, HandJointName.IndexMetacarpal)) { if (t != null) { return t; } }
            foreach (var t in m_modelJoints.ElementsFrom(HandJointName.RingTip, HandJointName.RingMetacarpal)) { if (t != null) { return t; } }
            foreach (var t in m_modelJoints.ElementsFrom(HandJointName.PinkyTip, HandJointName.PinkyMetacarpal)) { if (t != null) { return t; } }
            foreach (var t in m_modelJoints.ElementsFrom(HandJointName.ThumbTip, HandJointName.ThumbTrapezium)) { if (t != null) { return t; } }
            return null;
        }
        ...
        {
            foreach (var index in EnumArrayBase<HandJointName>.BaseEnumKeysFrom(startJoint, endJoint))
            {
        ...
        private void UpdatePoses()
        {
            ...
            {
                foreach (var joint in deviceState.readOnlyHandJoints)
                {
                    var index = joint.Key;
        ...
        private static float CalculateJointLength(JointEnumArray.IReadOnly joints)
        {
            ...
            foreach (var jointPose in joints.ElementsFrom(HandJointName.MiddleMetacarpal, HandJointName.MiddleTip))
            {
